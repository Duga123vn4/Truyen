...
class NovelProject:
    ...
    def get_glossary_content(self) -> str:
        """Nạp Core Canon tinh gọn (Index, Characters, Terms, Factions, Timeline) và chỉ lấy 5 sự kiện gần nhất."""
        core_files = [
            "ENTITY_INDEX.md",
            "characters.md",
            "terms.md",
        ...
        if self.glossary_dir.exists():
            for fname in core_files:
                ...
                if f.exists():
                    try:
                        ...
                        if txt:
                            glossary_texts.append(f"### File: {fname}\n{txt}")
                    except Exception:
                        pass
            # Nạp locations.md nếu có
            loc_file = self.glossary_dir / "locations.md"
            if loc_file.exists():
                try:
                    loc_txt = loc_file.read_text(encoding="utf-8").strip()
                    if loc_txt:
                        glossary_texts.append(f"### File: locations.md\n{loc_txt}")
                except Exception:
                    pass

            # Với events.md: Chỉ trích xuất 5 sự kiện gần nhất thay vì 155 KB lịch sử cũ
            ...
            if ev_file.exists():
                try:
                    ...
                    if recent_events:
                        clean_recent = "### " + "\n\n### ".join(recent_events)
                        glossary_texts.append(f"### File: events.md (5 Sự kiện gần nhất)\n{clean_recent}")
                except Exception:
                    pass
        return "\n\n".join(glossary_texts)
    ...
    def auto_update_glossary(self, extracted_data: Dict, chapter_num: int, raw_text: str = "") -> Dict[str, int]:
        """Canon Engine V3: Tích hợp canon_validator.py thuần Python. Hỗ trợ cập nhật Card thực thể cũ, Terms và Relationship Timeline."""
        ...
        for line in index_txt.splitlines():
            m = re.search(r'\|\s*`([A-Z]+-\d+)`\s*\|\s*\*\*([^*]+)\*\*\s*\|\s*([^|]+)\s*\|\s*([^|]+)\s*\|\s*`?([^|`]+)`?\s*\|\s*Tập\s*(\d+)', line)
            if m:
                existing_index_entries.append(EntityIndexEntry(
                    entity_id=m.group(1).strip(),
                    standard_name=m.group(2).strip(),
                    original_name=m.group(3).strip(),
                    entity_type=m.group(4).strip(),
                    status=m.group(5).strip(),
                    first_chapter=int(m.group(6))
                ))
        ...
        def update_existing_character_card(target_file: Path, entity_id: str, new_job: str = "", new_role: str = "", new_note: str = "", new_evidence: str = "", chapter: int = 1) -> bool:
            """Cập nhật thông tin mới (Thiên chức, Thân phận, Mô tả) vào Card nhân vật đã có."""
            if not target_file.exists():
                return False
            c_text = target_file.read_text(encoding="utf-8")
            pattern = re.compile(rf'(##\s*\[{entity_id}\][\s\S]*?)(?=\n##\s*\[|\Z)')
            m_card = pattern.search(c_text)
            if not m_card:
                return False
            card_block = m_card.group(1)
            updated_card = card_block
            changed = False

            if new_job and new_job.strip() not in ["Chưa rõ", "chưa rõ", ""] and "thiên_chức:** Chưa rõ" in card_block:
                updated_card = re.sub(r'(\-\s*\*\*thiên_chức:\*\*)\s*Chưa rõ', rf'\1 {new_job.strip()}', updated_card)
                changed = True
            elif new_job and new_job.strip() not in ["Chưa rõ", "chưa rõ", ""] and new_job.strip() not in card_block:
                updated_card = re.sub(r'(\-\s*\*\*thiên_chức:\*\*.*)', rf'\1 / {new_job.strip()}', updated_card)
                changed = True

            if new_role and new_role.strip() not in ["Nhân vật mới", "Chưa rõ", ""] and new_role.strip() not in card_block:
                updated_card = re.sub(r'(\-\s*\*\*thân_phận:\*\*.*)', rf'\1 / {new_role.strip()}', updated_card)
                changed = True

            if new_note and new_note.strip() and new_note.strip() not in card_block:
                updated_card = re.sub(r'(\-\s*\*\*mô_tả:\*\*.*)', rf'\1\n  • (Tập {chapter}): {new_note.strip()}', updated_card)
                changed = True

            if new_evidence and new_evidence.strip() and new_evidence.strip() not in card_block:
                updated_card = re.sub(r'(\-\s*\*\*bằng_chứng:\*\*.*)', rf'\1\n  • (Tập {chapter}): {new_evidence.strip()}', updated_card)
                changed = True

            if changed:
                updated_card = re.sub(r'(\-\s*\*\*trạng_thái_nhân_vật:\*\*.*)', rf'- **trạng_thái_nhân_vật:** BÌNH THƯỜNG (Cập nhật Tập {chapter})', updated_card)
                new_content = c_text[:m_card.start()] + updated_card + c_text[m_card.end():]
                target_file.write_text(new_content, encoding="utf-8")
                return True
            return False

        # 1. Xử lý Nhân vật mới & Cập nhật Nhân vật cũ
        ...
        if new_chars:
            ...
            for c in new_chars:
                ...
                job = (c.get("thien_chuc") or "").strip()
                role = (c.get("vai_tro") or "").strip()
                note = (c.get("ghi_chu") or "").strip()
                bang_chung = (c.get("bang_chung") or "").strip()
                if not name_vi and not name_orig:
                    continue

                val_res = validate_single_entity_extraction(
                    c,
                    raw_text=raw_text,
                    current_chapter=chapter_num,
                    is_chapter_translated=True,
                    existing_index=existing_index_entries
                )

                if val_res.action == "REJECT_HALLUCINATION":
                    console.print(f"  [bold red]⚠️ [HALLUCINATION BỊ CHẶN] {val_res.standard_name} ({val_res.original_name}): Bằng chứng không có trong RAW -> Loại bỏ.[/bold red]")
                    continue

                if val_res.log_evidence:
                    evidence_entries.append({
                        "loai": "nhan_vat",
                        "ten_vi": val_res.standard_name or name_vi,
                        "ten_goc": val_res.original_name or name_orig,
                        "chuong": chapter_num,
                        "thoi_gian": now_str,
                        ...
                        "temporal_scope": val_res.temporal_scope
                    })

                if val_res.action == "EVIDENCE_LOG_ONLY":
                    audit_entries.append(f"• 🔵 **[LOG]** **{val_res.standard_name}** ({val_res.original_name}): {c.get('loai_bang_chung')} — Chỉ lưu Sổ cái bằng chứng (Tập {chapter_num})")
                    continue

                if val_res.action == "SEND_TO_REVIEW":
                    review_entries.append({**c, "loai": "nhan_vat_review", "chuong": chapter_num, "thoi_gian": now_str, "status": val_res.status})
                    counts["review_queue"] += 1
                    audit_entries.append(f"• 🟡 **[REVIEW]** **{val_res.standard_name}** ({val_res.original_name}): {val_res.status} — Cần review (Tập {chapter_num})")
                    continue

                if val_res.action == "SKIP_CANON":
                    # Entity đã tồn tại -> Cập nhật thông tin mới (Thiên chức, Mô tả) vào Card cũ
                    matched_id = val_res.entity_id
                    target_file = gods_file if matched_id and matched_id.startswith("GOD") else (animals_file if matched_id and matched_id.startswith("ANIMAL") else char_file)
                    was_updated = update_existing_character_card(target_file, matched_id, new_job=job, new_role=role, new_note=note, new_evidence=bang_chung, chapter=chapter_num)
                    if was_updated:
                        audit_entries.append(f"• 🔄 **[CẬP NHẬT]** `{matched_id}` **{val_res.standard_name}**: Cập nhật thông tin mới (Thiên chức/Vai trò: {job or role}) (Tập {chapter_num})")
                    continue

                if val_res.action == "UPGRADE_MERGE":
                    audit_entries.append(f"• 🔄 **[NÂNG CẤP TÊN]** `{val_res.entity_id}`: **{val_res.standard_name}** ({val_res.original_name}) (Tập {chapter_num})")
                    continue

                if val_res.action == "ADD_ALIAS":
                    audit_entries.append(f"• 🏷️ **[BÍ DANH MỚI]** `{val_res.entity_id}`: **{val_res.aliases}** (Tập {chapter_num})")
                    continue

                # CREATE_CANON: Phân loại thông minh (Tránh nhầm 'Thần tượng' thành 'Thần linh')
                role_val = role or "Nhân vật mới"
                job_val = job or "Chưa rõ"
                status_val = c.get("trang_thai") or "CÒN SỐNG"
                note_val = note or ""
                bang_chung_val = bang_chung or ""
                do_tin_cay_val = c.get("do_tin_cay") or "XÁC NHẬN"

                all_check = (role_val + " " + job_val + " " + name_vi).lower()
                is_idol = any(w in all_check for w in ["thần tượng", "tinh thần", "thần tốc"])
                is_god = any(w in all_check for w in ["vị thần", "nữ thần", "chú thần", "tối cao thần", "thần linh", "thần giám hộ", "god", "deity"]) and not is_idol
                ...
                card = (f"\n---\n\n## [{eid}] {name_vi}\n\n"
                        ...
                        f"- **độ_tin_cậy:** {do_tin_cay_val}\n- **khóa_bảo_vệ:** CÓ\n"
                        f"- **trạng_thái_nhân_vật:** {status_val} (Cập nhật Tập {chapter_num})\n"
                        f"- **thân_phận:** {role_val}\n- **thiên_chức:** {job_val}\n"
                        f"- **nguồn:** Tập {chapter_num}\n"
                        f"- **bằng_chứng:** {bang_chung_val}\n"
                        f"- **mô_tả:** {note_val}\n")
                cur_tf = target_f.read_text(encoding="utf-8") if target_f.exists() else f"# HỒ SƠ {itype.upper()}\n"
                target_f.write_text(cur_tf.rstrip() + "\n" + card, encoding="utf-8")

                new_index_rows.append(f"| `{eid}` | **{name_vi}** | {name_orig} | {itype} | `{status_val}` | Tập {chapter_num} |")
                audit_entries.append(f"• 🟢 **[CANON]** `{eid}` **{name_vi}** ({name_orig}): {role_val} (Thiên chức: {job_val}) [{do_tin_cay_val}] (Tập {chapter_num})")
                counts["characters"] += 1

                existing_index_entries.append(EntityIndexEntry(
                    entity_id=eid, standard_name=name_vi, original_name=name_orig,
                    entity_type=itype, status=status_val, first_chapter=chapter_num
                ))

        # 2. Xử lý Thuật ngữ & Kỹ năng (Hỗ trợ cả thuat_ngu_moi và new_terms)
        raw_terms = extracted_data.get("thuat_ngu_moi") or extracted_data.get("new_terms") or []
        if raw_terms:
            cur_terms = terms_file.read_text(encoding="utf-8") if terms_file.exists() else "# ⚔️ THUẬT NGỮ & KỸ NĂNG\n"
            for t in raw_terms:
                term_vi = (t.get("ten_vi") or t.get("term_vi") or "").strip()
                term_orig = (t.get("ten_goc") or t.get("term_orig") or "").strip()
                ...
                t_type = (t.get("loai") or t.get("type") or "").lower()
                desc = t.get("mo_ta") or t.get("description") or ""
                bang_chung_t = t.get("bang_chung") or t.get("evidence") or ""
                all_text_check = (t_type + " " + desc + " " + term_vi).lower()

                # SCHEMA VALIDATION TẠI NGUỒN (3 TẦNG: EXACT + HEAD NOUN + LOUD OBSERVABILITY)
                from canon_validator import classify_entity_type_robust
                classified_info = classify_entity_type_robust(t_type)

                if classified_info:
                    pfx, itype, f_target_name = classified_info
                    if pfx == "RACE":
                        race_count = len(re.findall(r'RACE-\d+', index_txt)) + 1
                        eid = f"RACE-{race_count:03d}"; target_f = terms_file
                    elif pfx == "MONSTER":
                        monster_count = len(re.findall(r'MONSTER-\d+', index_txt)) + 1
                        eid = f"MONSTER-{monster_count:03d}"; target_f = terms_file
                    elif pfx == "ITEM":
                        eid = f"ITEM-{item_count:03d}"; item_count += 1; target_f = terms_file
                    elif pfx == "SKILL":
                        eid = f"SKILL-{skill_count:03d}"; skill_count += 1; target_f = terms_file
                    elif pfx == "JOB":
                        job_count = len(re.findall(r'JOB-\d+', index_txt)) + 1
                        eid = f"JOB-{job_count:03d}"; target_f = terms_file
                    elif pfx == "ORG":
                        eid = f"ORG-{org_count:03d}"; org_count += 1; target_f = orgs_file
                    elif pfx == "PLACE":
                        eid = f"PLACE-{place_count:03d}"; place_count += 1; target_f = places_file
                    elif pfx == "EVENT":
                        eid = f"EVENT-{event_count:03d}"; event_count += 1; target_f = events_file
                    else:
                        eid = f"TERM-{term_count:03d}"; term_count += 1; target_f = terms_file
                else:
                    # LOUD FAILURE: CẤM TỰ Ý NHÉT VÀO TERM-xxx! Ghi nhận vào unclassified_entities.jsonl
                    unclassified_file = self.glossary_dir / "unclassified_entities.jsonl"
                    unclass_entry = {"ten_vi": term_vi, "ten_goc": term_orig, "loai_la": t_type, "mo_ta": desc, "chuong": chapter_num, "thoi_gian": now_str}
                    with open(unclassified_file, "a", encoding="utf-8") as uf:
                        uf.write(json.dumps(unclass_entry, ensure_ascii=False) + "\n")
                    console.print(f"  [bold yellow]⚠️ [LOUD OBSERVABILITY] Phát hiện thực thể loại lạ ngoài Schema: '{term_vi}' (loại: '{t_type}') ➔ Chuyển vào Hàng chờ Duyệt (unclassified_entities.jsonl).[/bold yellow]")
                    continue

                card = f"\n---\n\n## [{eid}] {term_vi}\n\n- **id:** {eid}\n- **loại:** {itype.upper()}\n- **tên_chuẩn:** {term_vi}\n- **tên_gốc:** {term_orig}\n- **trạng_thái:** ĐÃ XÁC NHẬN\n- **canon:** CHÍNH THỨC\n- **độ_tin_cậy:** TUYỆT ĐỐI\n- **khóa_bảo_vệ:** CÓ\n- **nguồn:** Tập {chapter_num}\n- **bằng_chứng:** {bang_chung_t}\n- **mô_tả:** {desc}\n"
                cur_tf = target_f.read_text(encoding="utf-8") if target_f.exists() else f"# HỒ SƠ {itype.upper()}\n"
                target_f.write_text(cur_tf.rstrip() + "\n" + card, encoding="utf-8")
        ...
        # 3. Xử lý Quan hệ Dòng thời gian (relationship_timeline.md)
        raw_relations = extracted_data.get("quan_he") or extracted_data.get("relations") or []
        if raw_relations:
            cur_rel = rel_file.read_text(encoding="utf-8") if rel_file.exists() else "# ⏳ BIẾN ĐỘNG QUAN HỆ THEO THỜI GIAN\n| Tập | Nguồn | Loại Quan Hệ | Đích | Độ Tin Cậy | Bằng Chứng |\n| :---: | :--- | :---: | :--- | :---: | :--- |\n"
            rel_lines = []
            for r in raw_relations:
                src = (r.get("nguon") or r.get("source") or "").strip()
                rel_t = (r.get("loai_quan_he") or r.get("relation") or "").strip()
                dst = (r.get("dich") or r.get("target") or "").strip()
                ev = (r.get("bang_chung") or r.get("evidence") or "").strip()
                conf = (r.get("do_tin_cay") or r.get("confidence") or "XÁC NHẬN").strip()
                if src and dst and rel_t:
                    rel_lines.append(f"| Tập {chapter_num} | **{src}** | {rel_t} | **{dst}** | `{conf}` | {ev[:60] if ev else 'Trực tiếp'} |")
                    counts["relations"] += 1
            if rel_lines:
                rel_file.write_text(cur_rel.rstrip() + "\n" + "\n".join(rel_lines) + "\n", encoding="utf-8")
                audit_entries.append(f"• ⏳ **[QUAN HỆ]** Ghi nhận {len(rel_lines)} mối quan hệ mới (Tập {chapter_num})")

        # 4. Cập nhật Index
        if new_index_rows:
            index_file.write_text(index_txt.rstrip() + "\n" + "\n".join(new_index_rows) + "\n", encoding="utf-8")

        # 5. Cập nhật Sự kiện cốt truyện
        ...
        # 8. Ghi Sổ cái bằng chứng (evidence_log.jsonl)
        if evidence_entries:
            with open(evidence_log_file, "a", encoding="utf-8") as ef:
                for entry in evidence_entries:
                    ef.write(json.dumps(entry, ensure_ascii=False) + "\n")

        # 9. Ghi Hàng chờ duyệt (review_queue.jsonl)
        if review_entries:
            with open(review_queue_file, "a", encoding="utf-8") as rf:
                for entry in review_entries:
                    rf.write(json.dumps(entry, ensure_ascii=False) + "\n")
            console.print(f"  [bold yellow]⚠️ {len(review_entries)} thực thể chưa đủ bằng chứng -> Đưa vào Hàng chờ duyệt ({review_queue_file.name})[/bold yellow]")

        if counts["characters"] > 0:
            names = [c.get("ten_vi", c.get("name_vi", "")) for c in new_chars if c.get("ten_vi") or c.get("name_vi")]
            self.append_project_log("Canon Database V3", f"Tập {chapter_num}: Thêm mới {counts['characters']} nhân vật ({', '.join(names[:3])})")
        if counts["terms"] > 0:
            ...
            self.append_project_log("Canon Database V3", f"Tập {chapter_num}: Thêm mới {counts['terms']} thực thể ({', '.join(terms_names[:3])})")
        if counts["relations"] > 0:
            self.append_project_log("Canon Database V3", f"Tập {chapter_num}: Ghi nhận {counts['relations']} mối quan hệ thời gian")
        if counts["review_queue"] > 0:
            self.append_project_log("Canon Review Queue", f"Tập {chapter_num}: {counts['review_queue']} thực thể đưa vào Hàng chờ duyệt")
...
def scan_all_projects() -> List[NovelProject]:
    projects = []
    seen_paths = set()
    p_dir = ROOT_DIR / "projects"
    scan_dirs = [p_dir, ROOT_DIR] if p_dir.exists() else [ROOT_DIR]
    
    for s_dir in scan_dirs:
        if not s_dir.exists(): continue
        for item in s_dir.iterdir():
            if item.is_dir() and not item.name.startswith(".") and item.name not in ["tools", "audio", "scratch", "web", "assets", "backups", "__pycache__", ".obsidian", "projects"]:
...
{{
  "nhan_vat_moi": [
    {{
      ...
      "khang_dinh": [
        {{
          "noi_dung": "Mệnh đề khẳng định cụ thể về nhân vật",
          "bang_chung": "Câu trích dẫn tiếng Nhật trong RAW",
          "loai_bang_chung": "TRỰC TIẾP",
          "nguon_phat_ngon": "NGƯỜI_KỂ_CHUYỆN",
          "pham_vi_thoi_gian": {{"loai": "HIỆN_TẠI", "tu_chuong": null, "den_chuong": null}},
          "do_tin_cay": "XÁC NHẬN"
        }}
      ]
    }}
...
"""
    ...
    if glossary:
        base_prompt += f"\n\n### 📚 CANON HIỆN TẠI (CORE CANON CHUẨN XÁC — BẢO VỆ XƯNG HÔ & THUẬT NGỮ):\n{glossary}\n"
...
async def translate_chapter_interactive(project: NovelProject, raw_item: RawChapterInfo, api_key: str, model: str):
    ...
    # Cập nhật glossary
    project.auto_update_glossary(extracted_data, ep_num, raw_text=raw_content if "raw_content" in locals() else (raw_item.path.read_text(encoding="utf-8") if "raw_item" in locals() and raw_item.path.exists() else ""))
            ...
            async def translate_worker(raw_item: RawChapterInfo, index: int):
                ...
                async with semaphore:
                    try:
                        ...
                        async with glossary_lock:
                            counts = project.auto_update_glossary(extracted_data, ep_num, raw_text=raw_content if "raw_content" in locals() else (raw_item.path.read_text(encoding="utf-8") if "raw_item" in locals() and raw_item.path.exists() else ""))
...
async def select_project(projects: List[NovelProject], force_menu: bool = False) -> Optional[NovelProject]:
    while True:
        ...
        console.print(Panel.fit(
            "[bold cyan]🚀 HỆ THỐNG DỊCH THUẬT & QUẢN TRỊ CANON NOVEL AI[/bold cyan] [bold magenta]— V3.0 ULTIMATE[/bold magenta]\n"
            "[dim]🏛️ Kiến trúc: Tam Quyền Phân Lập • Mệnh Đề Nguyên Tử (Atomic Claims) • Chống Ảo Giác[/dim]",
...
async def main():
    ...
    while True:
        ...
        console.clear()
        console.print(Panel.fit(
            f"[bold cyan]🚀 HỆ THỐNG DỊCH THUẬT & QUẢN TRỊ CANON NOVEL AI — V3.0 ULTIMATE[/bold cyan]\n"
            f"[bold green]📖 Bộ truyện:[/bold green] [bold yellow]{project.name}[/bold yellow] • [bold magenta]Engine:[/bold magenta] [bold white]Canon DB V3.0 (Atomic Claims & Anti-Hallucination)[/bold white]\n"
            f"[dim]Hỗ trợ Google Gemini & Relay API • Sinh Ảnh Anime Tự Động • Đồng Bộ Web Reader[/dim]",
            border_style="bright_blue"
        ))
